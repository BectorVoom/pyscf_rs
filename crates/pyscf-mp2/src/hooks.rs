...
pub trait Mp2OverrideHooks {
    ...
    /// This trait-default seam returns `NotYetImplemented{plan:4}` because the
    /// supported route is the PyO3 bridge (05-07): it runs
    /// `rmp2_kernel(.., with_t2=true)` then `crate::rdm::make_rdm1(&res.t2, ..)`.
    /// As of 05-08 the `int2e` gate (cintx#11) is closed, so that bridge path
    /// produces a real `t2` and a real RDM end-to-end; this hook default stays a
    /// stub for callers that have not run the kernel themselves.
    ...
    /// ships as [`crate::rdm::make_rdm2`]; this trait-default seam is a
    /// `NotYetImplemented{plan:4}` stub for the same reason (the bridge path
    /// runs the kernel for `t2`). The `int2e` gate is closed as of 05-08.
    ///
