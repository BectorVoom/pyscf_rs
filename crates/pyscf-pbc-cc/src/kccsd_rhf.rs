...
#[derive(Debug, Clone)]
pub struct KAmplitudeSubspace {
    flat: Vec<f64>,
}

impl KAmplitudeSubspace {
    #[must_use]
    pub fn from_amplitudes(t1: &ZArr, t2: &ZArr) -> Self {
    ...
    #[must_use]
    pub fn to_amplitudes(&self, t1: &ZArr, t2: &ZArr) -> (ZArr, ZArr) {
        let n1 = t1.len();
        let n2 = t2.len();
        let mut a = t1.clone();
        let mut b = t2.clone();
    ...
    #[must_use]
    pub fn residual(&self, prev: &Self) -> Vec<f64> {
        self.flat
            .iter()
            .zip(prev.flat.iter())
            .map(|(a, b)| a - b)
