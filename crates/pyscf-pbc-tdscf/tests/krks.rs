...
fn hybrid_scaling_exactly_linear() {
    ...
    for i in 0..a0.len() {
        ...
        assert!(
            (d1 - d2).abs() < 1e-12,
            "hyb non-linearity at {i}: {d1:e} vs {d2:e}"
        );
    }
...
fn gate_a_uks_pbe() {
    ...
    let (a_hf, _) = build_uab(
        &args.0, &args.1, &args.2, &args.3, &args.4, d, nmo_a, nmo_b, 0.0,
    )
    .unwrap();
    let au = [
        flat(&r["a"][0]["re"]),
...
fn kuks_single_k_matches_uks() {
    ...
    let (a_hf, _) = pyscf_pbc_tdscf::uhf::build_uab(
        &args.0, &args.1, &args.2, &args.3, &args.4, d, nmo_a, nmo_b, 0.0,
    )
    .unwrap();
    let au = [
    ...
    let out = kernel_kuks_tda(
        &wrap(to_7d(args.0.clone(), d.nocc_a, nmo_a)),
        &wrap(zaa.clone()),
        &wrap(to_7d_ab(args.1.clone(), d.nocc_a, nmo_a, nmo_b)),
        &wrap(zab.clone()),
        &wrap(to_7d(args.2.clone(), d.nocc_b, nmo_b)),
        &wrap(zbb.clone()),
        &fxc_full,
        &fxc_im,
        &e_oa,
