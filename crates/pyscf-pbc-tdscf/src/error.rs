...
pub enum PbcTdscfError {
    ...
    #[error(
        "tdscf: Davidson did not converge in {cycles} cycles (last residual {last_residual:e})"
    )]
    ...
    #[error(
        "tdscf: A-B not positive definite (lowest {lowest:e}) - unstable reference, route to stability analysis"
    )]
