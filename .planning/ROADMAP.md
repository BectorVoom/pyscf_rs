...
- [ ] **Phase 11: FFT + FFTDF + periodic HF** — complex 3-D FFT (GEMM engine, then Stockham cubecl kernel), `get_coulG`/`madelung`/exxdiv, `fft_jk` J and K builds, `FFTDF`, `KSCF` driver (global aufbau, k-stacked DIIS), `KRHF`/`KUHF`/`KROHF`/`KGHF` + gamma-point variants, smearing, addons, chkfile. **Gate:** `KRHF(diamond, 2×2×2)` matches upstream to 1e-7 Ha; supercell-equivalence identity holds.
