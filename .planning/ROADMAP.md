...
- [ ] **Phase 15: Periodic AO2MO + KMP2** — `kconserv` tables, `KptsHelper`, `ktensor`, periodic MO transforms per DF backend, `KMP2`/`KUMP2`/`kmp2_stagger`. **Gate:** `KMP2` `e_corr` matches upstream to 1e-14.
...
- [ ] **Phase 17: k-point symmetry + multigrid** — `pbc/symm` (lattice/space-group detection, point groups, tables), `KPoints` IBZ machinery, every `*_ksymm` adapter (SCF/DFT/MP2/CCSD), `dft/multigrid`. **Gate:** symmetry-restricted energies equal full-BZ energies to 1e-14.
