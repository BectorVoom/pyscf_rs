...
- [ ] **Phase 13: ft_ao + AFTDF** *(IMPLEMENTED 2026-08-29 for plans 13-01…13-05 + 13-07; 13-06 PARTIALLY shipped — see `.planning/phases/13-ft-ao-aftdf/13-VERIFICATION.md`. **Reconciled 2026-09-13 (20-01): box stays `[ ]`** — `13-VERIFICATION.md:3-6` states implementation of those plans, not closure of the phase (Gate 3 "near-met for `get_pp`"), and the directory holds 8 PLAN files and 0 SUMMARY files. The `ao2mo_7d` carry-over below was since CLOSED by 14-05 (`14-VERIFICATION.md §10`). Floors: `.planning/phases/20-pbc-python-bindings/measurements/README.md`)* — K-15 `ft_aopair` McMurchie–Davidson cubecl kernel (concrete `f64`: `cube_math::double` IS the f64 libm, as in every sibling PBC kernel), host MD recursion, `estimate_rcut` + upstream's three screens, single-centre `ft_ao` + `_fake_nuc`, `FtKernel`, `AFTDF` (`ft_loop`/`weighted_coulG`/`get_nuc`/`get_pp`), `aft_jk`, and the `Box<dyn PeriodicDf>` refactor across all 8 drivers. **Gate 1 (no oracle) MET, in three parts** — the flat 1e-10 was unmeasured and unreachable: `ft_ao.estimate_rcut` (20.420 Bohr) is LOOSER than `cell.rcut` (21.319), so upstream's own `ft_aopair[G=0]` misses `int1e_ovlp` by 1.554e-9, and past `rcut` ×1.5 the FT sum is converged (×2.0 identical to four digits) while the residual sticks at 1.472e-10 because it is then `pbc_intor`'s OWN truncation. So: **1a ≤2e-9** vs `pbc_intor` at `estimate_rcut`, **1b ≤2e-10** at 1.5×`cell.rcut`, **1c ≤1e-13** vs `intor_cross_with_images` over the SAME image list — 1c passes on diamond, He-fcc and all 8 k-points and is the real gate on the algebra. **Gate 2 restated as a `(rcut, mesh)` ladder** — upstream measured: the KRHF energy difference plateaus at **2.607e-11 Ha**, BIT-IDENTICAL at mesh 31 and 41 (all 14 digits), and separately plateaus at `rcut` ×1.5; two independent floors, so both "1e-13" and any monotone-in-mesh restatement would fail a CORRECT implementation. Also measured: with `exxdiv='ewald'` the R-15 G=0 asymmetry is ~96% of `dvk` at mesh 31, yet the ENERGY floors with `dvj` — the G=0 term is a near-uniform shift that cancels in `Tr(D·vk)`, so matrices and energies need DIFFERENT tolerances. **Gate 2 MET in the port:** `KRHF` on diamond at gamma, both builders, `conv_tol=1e-11` — 9.309e-5 → 5.066e-8 → **2.378e-10** over meshes 15/21/27, the same ~1000×-per-6-mesh rate as upstream. **Gate 3 MET for 4 of 5 quantities:** `get_nuc` **2.755e-12**, `vj` **3.733e-12**, `vk` **2.116e-12**, `get_eri` **4.172e-12** — all beating the 1e-11 bar, which disproves the obvious hypothesis that `ft_aopair`'s screening residual propagates broadly. `get_pp` is the exception at 1.806e-9, and **the cause is upstream, not the port**: `aft.get_pp` builds part 2 with `_IntPPBuilder` while `pp_int.get_pp_loc_part2` is the reference route Phase 10 ported and `fft.get_pp` agrees with, and **those two upstream routes disagree with EACH OTHER by 1.7933e-9** — 99.3% of the gap. Substituting the `pp_int` route into upstream's own `AFTDF.get_pp` collapses the deviation to **3.982e-11** (45×), and that substitution is asserted in the test suite so the attribution cannot rot. `ft_aopair`'s own 5.121e-10 vs upstream is separate and is screening (three upstream screens ported: `strip_basis`, `get_ovlp_mask` over the `_RangeSeparatedCell` per-primitive grouping, libcint `PTR_EXPCUTOFF`, taking it 1.553e-9 → 5.121e-10); oracle-free evidence it is truncation and not algebra is Gate 1c at 1e-13 plus the `get_pp` anti-Hermitian residue falling from 5.133e-11 at upstream `rcut` to **2.665e-15** at a converged one. **Four defects caught by the phase's own tests** (a per-record screen reapplied at the image-list threshold, worth 1.66e-7; `estimate_rcut`'s `cs` being the libcint coefficient rather than `gto_norm`; `Γ(1.5)` returning 1; Phase-10's F-order outputs added raw). **13-06 partially shipped:** `get_eri` + `get_ao_pairs_G` for both builders through one shared contraction (oracle 4.172e-12; AFT-vs-FFT converges 4.250e-5 → 5.427e-10 over meshes 11/21; 8-fold symmetry to 1.966e-12), but NOT `general`/`get_mo_pairs_G`/`ao2mo_7d` — `ao2mo_7d`'s index order is a contract with Phase 15's KMP2 and was deliberately not guessed. **Carry-overs:** `ao2mo_7d` still blocks Phase 15; the two Gate-3 residuals both land naturally in Phase 14 (`gdf_builder` needs the `ExtendedMole` machinery, and `incore.aux_e2` IS `_IntPPBuilder`); `kk_adapted_iter`/`group_by_conj_pairs` still unported; AFTDF band k-points deferred. **Worth reporting upstream:** `pp_int.get_pp_loc_part2` and `_IntPPBuilder.get_pp_loc_part2` disagree by 1.79e-9 in PySCF 2.12.1.
- [x] **Phase 14: GDF / MDF / RSDF / RSJK** *(CLOSED — `.planning/phases/14-gdf-mdf-rsdf-rsjk/14-VERIFICATION.md`: "four of five gates MET at close", Gate 3 MET 2026-08-30, REOPENED 2026-09-05 for two GDF defects found by Phase 15, both fixed (§11). **Gate 1b (diamond) is PARTIAL** — the flagship `make_j3c` wall time is unmeasured. 9 PLAN files, 8 SUMMARY files. Box ticked 2026-09-13 by 20-01)* — `incore` 3-center lattice-sum integrals, `gdf_builder` compensating-charge scheme, `GDF` with HDF5 `_cderi`, `df_jk`, `df_ao2mo`, `outcore`, `MDFPhase`, `rsdf_helper`/`rsdf_builder`/`RSDF`, `rsjk`. ~~**Gate:** every DF builder gives the same KRHF energy (sub-ulp; verbatim in `14-VERIFICATION.md §1`) with GDF under 20% of FFTDF memory.~~ **Superseded by the gates of `14-CONTEXT.md`, as measured (`14-VERIFICATION.md:16-23`):** **1** GDF vs upstream, all-electron He-fcc — MET, `KRHF` **2.750e-10**; **1b** the same on diamond — **PARTIAL**; **1c** `nao > 1` AND `ki != kj` (He/`6-31g` `[1,1,2]`) — MET **3.017e-9**; **2** MDF converges to FFTDF — MET, 1.695e-06 → 3.433e-09 → 3.245e-08 on upstream's ladder; **3** GDF vs RSDF — MET, RSDF **2.325e-10** / GDF **2.750e-10** vs upstream's own two routes; **4** `_cderi` under 20 % of the FFTDF AO table with the k-mesh pinned at 2×2×2 — MET, **6.08 %** (upstream 6.17 %, `14-VERIFICATION.md §4`). Still refused and carried: `rsjk` build/get_jk and `GDF/MDF.get_jk(omega)` (`20-CONTEXT.md §2(b)`). The original pre-implementation note follows. Measured upstream 2026-08-29 (`.planning/phases/14-gdf-mdf-rsdf-rsjk/measurements/`): GDF is an *approximation*, so `|E_FFTDF − E_GDF|` on diamond `gth-szv` 2x2x2 is **1.222e-03 Ha** — the DF **fitting error**, a property of the auxiliary basis, present in upstream too and reachable by no implementation. Upstream's OWN two GDF builders (`_RSGDFBuilder`, the `_prefer_ccdf=False` default, vs `_CCGDFBuilder`) disagree by up to **4.502e-06**. The pairs that CAN gate a port are **GDF vs RSDF** (upstream floor **1.353e-08**), **FFTDF vs MDF** (**1.124e-06** at MDF's default mesh, bottoming at **3.241e-10** at mesh 21 — the real convergence gate, and NOT monotone past it), and above all **the port vs upstream itself** (1e-11 on the all-electron He-fcc control; 3e-8 on diamond, where D-PBC-23 defers `exclude_dd_block` at a measured cost of **1.835e-8 Ha**). And the memory half is **k-mesh dependent**: GDF's `_cderi` is `O(nkpts^2)` against the FFTDF AO table's `O(nkpts)`, so at mesh `[40,40,40]` upstream itself is at **6.17%** for 2x2x2 but **20.95%** for 3x3x3 — the gate passes only with the k-mesh pinned at 2x2x2.
- [x] **Phase 15: Periodic AO2MO + KMP2** *(CLOSED 2026-09-05 — `.planning/phases/15-periodic-ao2mo-kmp2/15-VERIFICATION.md` "Verdict: CLOSED"; its one NOT MET row, the GDF route, was re-measured MET the same day after `14-VERIFICATION §11`. 8 PLAN files, 8 SUMMARY files. Box ticked 2026-09-13 by 20-01)* — `kconserv` tables, `KptsHelper`, `ktensor`, periodic MO transforms per DF backend, `KMP2`/`KUMP2`/`kmp2_stagger`. ~~**Gate:** `KMP2` `e_corr` matches upstream (the original unmeasured sub-ulp roadmap number).~~ **Gate as measured (15-01, `15-VERIFICATION.md:207`): `KMP2` `e_corr` vs upstream to 2e-6 Ha PER DF ROUTE — MET.** FFTDF diamond `gth-szv` `[1,1,2]` **5.418e-11** (`:39`); FFTDF He/`6-31g` **2.719e-11** (`:41`); GDF He/`6-31g` **4.090e-10** (`:42`); staggered KMP2 **1.408e-12** / **8.279e-13** (`:58-59`).
- [ ] **Phase 16: Periodic CC + CI** *(PARTIAL — `.planning/phases/16-periodic-cc-ci/16-VERIFICATION.md:3` "Status: PARTIAL". Its §6.1 table has since recorded 16-06, 16-09, 16-10, 16-11 and 16-12 CLOSED, but the status line was never updated and three gates remain NOT RUN (§6.2: G2 GDF/RSDF route, cross-thread determinism, the (T) peak-memory bound), so **the box stays `[ ]`** (reconciled 2026-09-13 by 20-01). 14 PLAN files, 12 SUMMARY files)* — `KRCCSD`/`KUCCSD`/`KGCCSD` + k-intermediates, `KCCSD(T)`, `EOM-KCCSD` IP/EA (EE: GHF full, RHF singlet only, UHF absent upstream — `16-VERIFICATION.md §6.3`), `KCIS`, `kuccsd_rdm`. Reuses the `pyscf-ccsd` `WorkspacePool` arena + `PYSCF_MAX_MEMORY` pre-flight refusal. ~~**Gate:** `KRCCSD` `e_corr` matches upstream (the original unmeasured sub-ulp roadmap number).~~ **Gates as measured (16-01, `16-periodic-cc-ci/measurements/README.md §1`; results `16-VERIFICATION.md §1`):** `KRCCSD e_corr` **1e-7 PER DF ROUTE** — G1 FFTDF MET at **6.560e-9**; **G2 GDF/RSDF NOT RUN**; `KGCCSD` **2.066e-9** (1e-8); `KUCCSD` **6.05e-10** (1e-7); (T) fast vs slow **8.363e-13** relative (1e-12); spin-orbital (T) vs upstream **3.459e-11** (1e-9); 28 EOM roots **6.86e-11 … 4.94e-9** (1e-5); `KCIS` roots **5.48e-10** (1e-5); incore vs spilled `_ERIS` bit-identical; `symm_map` vs all-triples **7.93e-7** (1e-6). The FFTDF/MDF vs GDF/RSDF `e_corr` split of **9.22e-04 Ha** is upstream disagreeing with itself (`measurements/README.md §4`), not a gate.
- [x] **Phase 17: k-point symmetry + multigrid** *(CLOSED 2026-09-07 — `.planning/phases/17-ksymm-multigrid/17-VERIFICATION.md`; thirteen plans, twelve shipped and measured (13 PLAN files, 12 SUMMARY files). **Two gates are NOT met and stay visible: GDF Gate C (`KRKS`) and Gate D.** Box ticked 2026-09-13 by 20-01)* — `pbc/symm` (lattice/space-group detection, point groups, tables), `KPoints` IBZ machinery, every `*_ksymm` adapter (SCF/DFT/MP2/CCSD), `dft/multigrid`. ~~**Gate:** symmetry-restricted energies equal full-BZ energies (the original unmeasured sub-ulp roadmap number).~~ **Five gates as measured (17-01) and achieved (`17-VERIFICATION.md §3`):** **A** IBZ integers 145/145/245/408/816/2052 EXACT on `si`/`diamond`; **B** transforms ≤1e-9 at default `cell.precision`, ≤1e-13 at `precision = 1e-13` (`:107`); **C** ksymm vs full BZ, mesh pinned, `si [2,2,2]`: FFTDF `KRHF` **8.793e-14**, FFTDF `KRKS` **3.109e-14 / 2.842e-14**, GDF `KRHF` **2.486e-10**, **GDF `KRKS` 1.432e-06 — NOT MET** against 1e-8 (`:123-126`, `:801`); **D** port vs upstream per DF route — **NOT RUN** as a Rust gate, a carry-over (`:806`); **E** multigrid v2 `get_j` vs FFTDF **1.24e-08** (diamond) / **6.80e-08** (si), at upstream's own floor (`:168-169`). Post-SCF: `KMP2` ksymm vs full BZ **1.138e-10** (FFTDF) / **5.632e-11** (GDF); `KRCCSD` ksymm vs this port's full BZ **4.838e-12** (`:198-199`, `:231`).
- [ ] **Phase 18: Periodic gradients + stress + geomopt** — derivative lattice-sum integrals, PP and Ewald nuclear gradients, `grad/{krhf,kuhf,krks,kuks,krkspu,kukspu}` + gamma variants, the four `*_stress` modules, periodic geometry optimization (atoms only — upstream `pyscf/pbc/geomopt` has no lattice degrees of freedom: `grep -rn lattice pyscf/pbc/geomopt/*.py` hits once, inside the `__main__` example block, so variable-cell relaxation is a post-v2.0 feature, not a port). *(**IMPLEMENTED 2026-09-20, NOT CLOSED** — `.planning/phases/18-periodic-gradients-stress-geomopt/18-VERIFICATION.md` is the authority: all 21 plans shipped (18-08 DFT+U gradients and 18-14 geomopt landed this session; 18-21 restated the gates with measured floors). Green: Gate B/C on KRHF(gamma)/KRKS/KUKS/closed-shell KUHF, Gate A tiers except two lif A1 closed-form comparisons, Gate D LDA, all k-point stress bodies, DFT+U component/wiring gates, geomopt machinery + refusals. RUN/FAIL with numbers: gamma Gate E (F-18-10-01), KRHF 1×1×2 Gate B (F-18-19-01), KUHF open-shell Gate B (F-18-06-01), lif A1 closed-form (F-18-12-01), 2-atom geomopt convergence (F-18-14-01). NOT RUN: full-SCF+U B/C (no DFT+U SCF driver). Box stays `[ ]` until the five findings close.)* ~~**Gate:** always-on `verify_fd` central difference agrees to a sub-ulp Ha/Bohr number; stress agrees with `dE/da` finite difference to a sub-ulp number.~~ **Restated by `18-CONTEXT.md §2.1-§2.3`:** the old gradient number is below the arithmetic floor of a central difference — the cancellation error alone is `2·ε·|E|/h ≈ 4e-10` for a diamond cell (`|E| ~ 10 Ha`, `h = 1e-5`) and the truncation term at upstream's DFT step is `~1e-6` — and it contradicts both `crates/pyscf-grad/src/verify_fd.rs:35` (`FD_TOL = 1e-6`) and upstream's own 6-decimal (5e-7) FD assertions; the stress number was in the wrong units (upstream's stress is a pressure, Ha/Bohr³). Five gates replace it, **RESTATED 2026-09-20 by 18-21 with measured floors** (`measurements/README.md`): **A** component strain derivatives vs this port's own FD, three tiers **A1 1e-9** (floor 9.91e-10 — 1.01×, razor-thin and stated) / **A2 2e-9** (floor 5.01e-10) / **A3 1e-8** (floor 3.42e-9); **B** analytic gradient vs `verify_fd` **1e-6 Ha/Bohr** (floors 5.28e-10 KRHF, 4.32e-10 PBE; **5e-6** for `krkspu`/`kukspu`); **C** vs upstream's published `lib.fp(g)` at upstream's own decimal count (residuals 1.7e-8 HF, ~1.5e-9 DFT, 8.3e-9 DFT+U); **D** stress vs FD of `E(ε)` at **1e-6 Ha/Bohr³** (`h = 1e-3`, `/vol` in the assertion — inherits upstream's own number, no separate floor measured); **E** the gamma-point multigrid-v2 bodies at **1e-8** (floors 9.1e-11 … 6.24e-9 lif; graphene excluded — upstream has no 2D Ewald nuclear gradient), never Gate B's number. **cintx status (retired 2026-09-13 by 18-03 per `18-CONTEXT §1.3`):** the five `_ip1_`/`_ip2` PP-gradient symbols ship under the existing `gth-pp` feature at the same maturity as the energy-half families, and the extended `cintx_moment_weighted_available.rs` re-proves the R-14 refusal for the derivative half — the Wave-0.5 framing is stale. FFTDF J/K gradients are grid-based (no `int2e_ip1`) and stress needs no derivative integrals.
- [ ] **Phase 19: Periodic response + relativistic** — CPHF (reusing the single Krylov solver, GRAD-10), `newton_ah`, `stability`, `tdscf` (k + gamma, HF and KS), `gw` (AC, CD, slow), `adc` (IP/EA), `x2c` (sfx2c1e, x2c1e), `eph`. ~~**Gate:** KRHF-TDA lowest excitation matches upstream to 1e-15 eV.~~ **MEASURED AND RESTATED 2026-09-12** (`19-01`, `measurements/README.md`): ~~1e-15 eV~~ is ~1 f64 ulp at 5 eV and ten orders tighter than upstream's own modal assertion (5dp). Five per-family gates replace it: **A1** HF-family TDA/TDHF roots 1e-5 (modal energy assertion, sorted within k-shift); **A2** KS-family roots 1e-8 Ha (tightest energy assertion); **B** response seam exercises `pyscf_grad::cphf::solve`, no second `pub fn solve` (structural); **C** G0W0 QP energies 1e-4 per route (AC ≠ CD); **D** ADC IP/EA roots 1e-4; **E** X2C one-electron energies 1e-8 Ha. **CLOSE-OUT 2026-09-13** (`19-VERIFICATION.md`): all 19 plans shipped; A1/A2/B/D/E MET; C MET for AC on a live fixture and for CD/UAC/slow on analytic-model arms + oracle-free identities — the three live-vs-upstream GW arms are NOT RUN (`.planning/carryovers/19-gw-live-oracle.md`). Checkbox stays `[ ]` until they run.
- [ ] **Phase 20: MPI + tools + bindings + ship** *(ROLLED UP 2026-09-14 by 20-18, **NOT CLOSED** — `.planning/phases/20-pbc-python-bindings/20-VERIFICATION.md`; started 2026-09-13 — `.planning/phases/20-pbc-python-bindings/`; 18 plans per `20-CONTEXT.md §5`, which replaces `PBC-MASTER-PLAN §8.12`'s eight: `mpicc` is DROPPED because it binds the stub `pyscf-pbc-mpi`, and `k2gamma`/lattice/`pyscf_ase` are Rust-porting work recorded as unported by 20-17)* — `k2gamma`, lattice DB, ASE bridge, optional wannier90 shim, `mpitools`/`mpicc` behind a default-OFF `mpi` feature, the full `pyscf.pbc.*` PyO3 surface with Phase-3 subclass-override semantics, upstream-test-suite oracle CI (≥80% pass), periodic benchmark suite. ~~**Gate:** an unmodified upstream `pyscf.pbc` script runs on pyscf-rs~~ — vacuously TRUE today with zero work, because `python/pyscf/pbc/__init__.py` is a `pkgutil.extend_path` passthrough to the vendored upstream tree (`20-CONTEXT.md §1.1`). **Gate (restated 2026-09-13 by 20-01, owned by 20-18): the identity contract `assert pyscf.pbc.scf.KRHF is pyscf._native.pbc.scf.KRHF` holds for all 18 names in `python/pyscf/tests/test_pbc_identity_gate.py`, AND the unmodified upstream scripts `examples/pbc/20-k_points_scf.py` and `examples/pbc/22-k_points_mp2.py` run with that identity holding, landing on the floors in `.planning/phases/20-pbc-python-bindings/measurements/README.md`.** The restated gate discriminates: on the pre-binding tree it fails 18/18 (`ModuleNotFoundError: No module named 'pyscf._native.pbc'`, `measurements/identity-gate-pre-20-09.out`). **Status (20-18, 2026-09-14):** the identity gate holds **18/18** on the final tree and failed 18/18 before any binding, so it discriminates. The example-script half of the gate was **restated** (orchestrator decision, `measurements/example-gate-gap-scope.md` §5) because upstream 2.12.1 itself cannot run it: `20-k_points_scf.py` needs periodic meta-GGA and a k-point Newton-AH (days upstream), and `22-k_points_mp2.py` exits `NotImplementedError` at line 62. As restated, legs 1–3 are MET: ex 22 runs natively to line 62 with KMP2 `e_tot` 4.558e-7 (2×2×2) / 6.543e-9 (1 k) from upstream (floor 2e-6); `22-k_points_mp2_ksymm.py` lands at 1.695e-6; `23-smearing.py` at 2.2e-9; every touched name is `which_impl == 'native'`. Leg 4, a bounded native replacement for example 20, is NOT MET: the only qualifying script, `40-custom_gdf.py`, was killed after 82 min natively, against 53.5 s upstream. KRHF He-fcc 2×2×2 is bit-identical at 1 and 8 rayon threads, and a new `pbc-oracle` CI step checks it. **NOT MET:** the upstream-test-suite target (≥80% pass) — **11 / 815** (control 808 / 815; 684 collection errors from upstream PBC Python subclassing the native, non-subclassable `gto.Mole` / `scf.hf.RHF`; counterfactual 49 / 815). Native FFTDF `get_jk` also runs at 5.95× upstream's wall time, and one GDF `get_jk` on that cell took more than 580 s. Carryovers: `.planning/carryovers/20-*.md` (13 files). The periodic benchmark suite, `k2gamma`, lattice DB, ASE bridge and `mpitools`/`mpicc` were not done. The box stays `[ ]`.
