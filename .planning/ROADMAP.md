...
- [ ] **Phase 10: Periodic integrals + GTH pseudopotentials** — `pbc_intor` lattice-sum 1e integrals with Bloch phases (image-expansion route over `cintx` shell pairs), neighbor list + screening, GTH PP (`vloc` part1/part2 + `vnl`), periodic `eval_ao_kpts`, `get_hcore`/`get_ovlp`. **Gate:** `pbc_intor('int1e_ovlp', kpts)` matches upstream to 1e-12 on diamond 2×2×2. **cintx prerequisite:** the 10 moment-weighted families (`int3c1e_r{2,4,6}_origk`, `int1e_r{2,4}_origi`) — cintx Wave 0.5 in `cintx/.planning/notes/gradient-family-gap-closure-PLAN.md` §1.3b. Without them GTH pseudopotentials do not evaluate and no periodic SCF runs.
...
- [ ] **Phase 12: Periodic DFT** — periodic `NumInt` (`eval_ao_kpts` block loop, complex `eval_rho`, `nr_rks`/`nr_uks`/fxc), uniform + Becke periodic grids, `KRKS`/`KUKS`/`KROKS`/`KGKS` + gamma variants, DFT+U, `numint2c`, cDFT, and the `dimension ∈ {0,1,2}` low-dimension closure. **Gate:** `KRKS(Si, 2×2×2, PBE)` matches upstream to 1e-7 Ha.
- [ ] **Phase 13: ft_ao + AFTDF** — the `ft_aopair` McMurchie–Davidson cubecl kernel (largest new kernel in v2.0), `ft_ao`, `AFTDF` `get_nuc`/`get_pp`/`get_jk`, `aft_jk`, `aft_ao2mo`. **Gate:** `ft_aopair[G=0] == pbc_intor('int1e_ovlp')`; AFTDF KRHF == FFTDF KRHF to 1e-6.
- [ ] **Phase 14: GDF / MDF / RSDF / RSJK** — `incore` 3-center lattice-sum integrals, `gdf_builder` compensating-charge scheme, `GDF` with HDF5 `_cderi`, `df_jk`, `df_ao2mo`, `outcore`, `MDF`, `rsdf_helper`/`rsdf_builder`/`RSDF`, `rsjk`. **Gate:** every DF builder gives the same KRHF energy to 1e-6 with GDF under 20% of FFTDF memory.
