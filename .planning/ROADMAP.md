...
- [ ] **Phase 12: Periodic DFT** — periodic `NumInt` (`eval_ao_kpts` block loop, complex `eval_rho`, `nr_rks`/`nr_uks`/fxc), uniform + Becke periodic grids, `KRKS`/`KUKS`/`KROKS`/`KGKS` + gamma variants, DFT+U, `numint2c`, cDFT, and the `dimension ∈ {0,1,2}` low-dimension closure. **Gate:** `KRKS(Si, 2×2×2, PBE)` matches upstream to 1e-12 Ha.
- [ ] **Phase 13: ft_ao + AFTDF** — the `ft_aopair` McMurchie–Davidson cubecl kernel (largest new kernel in v2.0), `ft_ao`, `AFTDF` `get_nuc`/`get_pp`/`get_jk`, `aft_jk`, `aft_ao2mo`. **Gate:** `ft_aopair[G=0] == pbc_intor('int1e_ovlp')`; AFTDF KRHF == FFTDF KRHF to 1e-12.
- [ ] **Phase 14: GDF / MDF / RSDF / RSJK** — `incore` 3-center lattice-sum integrals, `gdf_builder` compensating-charge scheme, `GDF` with HDF5 `_cderi`, `df_jk`, `df_ao2mo`, `outcore`, `MDF`, `rsdf_helper`/`rsdf_builder`/`RSDF`, `rsjk`. **Gate:** every DF builder gives the same KRHF energy to 1e-12 with GDF under 20% of FFTDF memory.
- [ ] **Phase 15: Periodic AO2MO + KMP2** — `kconserv` tables, `KptsHelper`, `ktensor`, periodic MO transforms per DF backend, `KMP2`/`KUMP2`/`kmp2_stagger`. **Gate:** `KMP2` `e_corr` matches upstream to 1e-12.
- [ ] **Phase 16: Periodic CC + CI** — `KRCCSD`/`KUCCSD`/`KGCCSD` + k-intermediates, `KCCSD(T)`, `EOM-KCCSD` IP/EA/EE (RHF/UHF/GHF), `KCIS`, `kuccsd_rdm`. Reuses the `pyscf-ccsd` `WorkspacePool` arena + `PYSCF_MAX_MEMORY` pre-flight refusal. **Gate:** `KRCCSD` `e_corr` matches upstream to 1e-12.
- [ ] **Phase 17: k-point symmetry + multigrid** — `pbc/symm` (lattice/space-group detection, point groups, tables), `KPoints` IBZ machinery, every `*_ksymm` adapter (SCF/DFT/MP2/CCSD), `dft/multigrid`. **Gate:** symmetry-restricted energies equal full-BZ energies to 1e-12.
- [ ] **Phase 18: Periodic gradients + stress + geomopt** — derivative lattice-sum integrals, PP and Ewald nuclear gradients, `grad/{krhf,kuhf,krks,kuks,krkspu,kukspu}` + gamma variants, the four `*_stress` modules, periodic geometry/lattice optimization. **Gate:** always-on `verify_fd` central difference agrees to 1e-12 Ha/Bohr; stress agrees with `dE/da` finite difference to 1e-5. **cintx status (2026-08-22):** every gradient family this phase needs already ships and is oracle-covered — the v1.0 Phase-7 "6 of 8 missing" claim is stale. FFTDF J/K gradients are grid-based (no `int2e_ip1`) and stress needs no derivative integrals. Only the PP-gradient half (`int3c1e_ip1_r{2,4,6}_origk`, `int1e_r{2,4}_origi_ip2`) waits on cintx Wave 0.5.
- [ ] **Phase 19: Periodic response + relativistic** — CPHF (reusing the single Krylov solver, GRAD-10), `newton_ah`, `stability`, `tdscf` (k + gamma, HF and KS), `gw` (AC, CD, slow), `adc` (IP/EA), `x2c` (sfx2c1e, x2c1e), `eph`. **Gate:** KRHF-TDA lowest excitation matches upstream to 1e-12 eV.
