...
- [ ] **Phase 18: Periodic gradients + stress + geomopt** — derivative lattice-sum integrals, PP and Ewald nuclear gradients, `grad/{krhf,kuhf,krks,kuks,krkspu,kukspu}` + gamma variants, the four `*_stress` modules, periodic geometry/lattice optimization. *(**IN PROGRESS, PAUSED** — `.planning/phases/18-periodic-gradients-stress-geomopt/18-IMPLEMENTATION-CHECKPOINT.md`, 2026-09-12: 21 PLAN files, 0 SUMMARY files, no VERIFICATION; substrate partially implemented and uncommitted in the working tree; analytic PP/Ewald gradients, FFTDF derivative J/K, method assemblies, stress and geometry optimization unimplemented. Paused on upstream `test_get_vxc_lda`'s 1.0564055741291156e-9 residual against its 1e-9 A1 bound; the checkpoint's later update attributes it to FD energy-reduction roundoff and adds a stable-summation reference mode, which "does not complete 18-16 or Phase 18". Row reconciled 2026-09-13 by 20-01)* ~~**Gate:** always-on `verify_fd` central difference agrees to a sub-ulp Ha/Bohr number; stress agrees with `dE/da` finite difference to a sub-ulp number.~~ **Restated by `18-CONTEXT.md §2.1-§2.3`:** the old gradient number is below the arithmetic floor of a central difference — the cancellation error alone is `2·ε·|E|/h ≈ 4e-10` for a diamond cell (`|E| ~ 10 Ha`, `h = 1e-5`) and the truncation term at upstream's DFT step is `~1e-6` — and it contradicts both `crates/pyscf-grad/src/verify_fd.rs:35` (`FD_TOL = 1e-6`) and upstream's own 6-decimal (5e-7) FD assertions; the stress number was in the wrong units (upstream's stress is a pressure, Ha/Bohr³). Five gates replace it: **A** component strain derivatives vs this port's own FD, three tiers **A1 1e-9 / A2 2e-9 / A3 1e-8**; **B** analytic gradient vs `verify_fd` **1e-6 Ha/Bohr** (**5e-6** for `krkspu`/`kukspu`); **C** vs upstream's published `lib.fp(g)` at upstream's own decimal count; **D** stress vs FD of `E(ε)` at **1e-6 Ha/Bohr³** (`h = 1e-3`, `/vol` in the assertion); **E** the gamma-point multigrid-v2 bodies gated against their own measured floor, not B/C's. 18-01's upstream floors are in `measurements/README.md`; its coordinated gate restatement is still pending 18-16/18-17/18-21. **cintx status (2026-08-22):** every gradient family this phase needs already ships and is oracle-covered — the v1.0 Phase-7 "6 of 8 missing" claim is stale. FFTDF J/K gradients are grid-based (no `int2e_ip1`) and stress needs no derivative integrals. Only the PP-gradient half (`int3c1e_ip1_r{2,4,6}_origk`, `int1e_r{2,4}_origi_ip2`) waits on cintx Wave 0.5.
