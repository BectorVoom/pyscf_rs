...
---
*Last updated: 2026-05-25 after Phase 6 (CCSD) completion — ccsd moved Active → Validated (CCSD-01..11; 11/11 plans). Verification human_needed: caffeine/cc-pVDZ + DF-spill + λ/RDM byte-identity + python3.13t = workflow_dispatch arms in 06-HUMAN-UAT.md; caffeine in-core also gated on an upstream cintx/SCF d-function issue (one-electron Rys fixed in cintx 13fe9d3; d-function SCF convergence is a separate tracked item). NOTE: dft (Phase 4) + mp2 (Phase 5) shipped but their Active→Validated move was not recorded here (pre-existing PROJECT.md drift). Next: Phase 7 (gradients + geomopt).*
