...
| **18** — PP gradient `vpploc_part2_nuc_grad` | `int3c1e_ip1_r2_origk`, `_r4_origk`, `_r6_origk` | ✅ ships under the existing `gth-pp` feature, same maturity as the energy-half families (`oracle_covered=false` both sides); `cintx_moment_weighted_available.rs` extended by 18-03 re-proves the R-14 refusal | ~~BLOCKS plan 18-01~~ **RETIRED by 18-CONTEXT §1.3** |
| **18** — PP gradient `vppnl_nuc_grad` | `int1e_ipovlp` | ✅ shipped | none |
| **18** — PP gradient `vppnl_nuc_grad` | `int1e_r2_origi_ip2`, `int1e_r4_origi_ip2` | ✅ ships under the existing `gth-pp` feature, same maturity as the energy-half families (`oracle_covered=false` both sides); `cintx_moment_weighted_available.rs` extended by 18-03 re-proves the R-14 refusal | ~~BLOCKS plan 18-01~~ **RETIRED by 18-CONTEXT §1.3** |
...
| **D-PBC-29** | **Phase 16's speed + memory ruling, four clauses.** **(1) Complex tensors get their own arena.** `ZWorkspacePool` in `pyscf-runtime` with `shape_bytes = product * 16`, non-copying block access and per-buffer locking; `WorkspacePool`'s f64 API stays byte-for-byte unchanged. Reinterpreting `Box<[f64]>` as complex pairs is FORBIDDEN. **(2) Contractions are host rayon loops over k-triples with `oracle_*` accumulators, not `zgemm_dense`**, and every site NAMES its primitive (`oracle_zdot` = `zdotc` vs `oracle_zdotu`). **(3) `symm_map` is used from the FIRST version of the ERI build**, not retrofitted. **(4) Storage tiers are selected from an exact per-tensor byte count, never from upstream's `_mem_usage`**, and at least one green test must cross a tier boundary. Recorded 2026-09-02, plans 16-02/16-05, `16-REVIEW.md §6`. **AMENDED 2026-09-06 by the 16-01 measurement** (`.planning/phases/16-periodic-cc-ci/measurements/README.md §7`): clause (3)'s "~4×" is **MEASURED at 2.10× wall clock** on diamond `gth-szv` 2×2×2 (`59.487 s` symmetry loop vs `125.029 s` all-triples), with a count ratio of 2.91 — 176 orbit representatives for 512 triples, rising to 3.87 only at `nkpts = 64`. Two reasons: the orbit collapses at fixed points, and `vvvv` is built by `ao2mo_7d` in BOTH paths (`kccsd_rhf.py:798`; the `self.vvvv[...]` line inside the symmetry loop is commented out) so it saves nothing. `16-REVIEW.md §3`'s own instruction — "if the measured ratio is materially below 4, say so" — is discharged here; the clause STANDS (`symm_map` from the first version) at `~2×`. Clause (4)'s derived over-estimate is CONFIRMED at a measured **9.143×** (`gth-szv`) / **6.058×** (`gth-dzvp`). A further measured correction the review did not anticipate: **the symmetry loop is not bit-identical to the all-triples loop** — upstream's own two paths differ by up to `1.32e-7` at `mesh = [15,15,15]`, so 16-05 test 5 gates at 1e-6. | **(1)** `workspace_pool.rs:278-280` computes `shape_bytes` as `product * 8` and feeds `try_reserve` (`:266-274`), the HARD `MemoryLimitExceeded` refusal — a complex tensor sized with `* 8` under-reports by 2× to the one mechanism whose job is to refuse before an OOM, on the machine where 17-12's host suite already exit-137s. `as_slice` (`:397`) is `Ok(b.to_vec())`, a full copy per access, and `with_mut_slice` (`:461-483`, the closure runs at `:475`) holds the pool's single `Mutex` across the caller's closure, which would serialise every rayon k-loop at one core. `pyscf-ccsd` has zero complex arithmetic, so only the arena's SHAPE is reusable, not its element type. **(2)** Standing measurements `zgemm-dense-loses-to-host-rayon` (6-12× slower on the CPU backend AND 1.35e-10 off, outside the 1e-11 gate) and `pyscf-algebra-cpu-is-default-backend`. Re-measured against this phase's own shapes at 16-14 Task 4.2, and amended if it wins there. The naming requirement is `15-REVIEW.md D-15-R-02`'s lesson: a plan saying only "route through `oracle_dot`" yields `Σ x·x` instead of `Σ conj(x)·y` — a plausible wrong number only the final energy gate catches. **(3)** `kpts_helper.py:583-630` puts each of the `nkpts³` triples in an orbit of ≤4; `kccsd_rhf.py:783`/`:798-805`/`:909` transforms `≈nkpts³/4` blocks and transposes the rest — a derived ~4× on the phase's dominant step, which the original §8.8 never mentions. **`15-REVIEW.md D-15-R-04`'s ≤2× ruling does NOT carry over**: it held because KMP2 only wants `(ov|ov)` blocks, whereas KCCSD's `_ERIS` wants the full general block (`kccsd_rhf.py:789-794`), so all four operations land inside the set it needs. Measured by 16-01 Task 6, not asserted cold. **(4)** `kccsd_rhf.py:1100-1107` is `nkpts³·nmo⁴·4·16` carrying its own `# TODO`, and over-estimates by **9.1×** (diamond `gth-szv`) to **6.2×** (`gth-dzvp`) against the seven blocks actually allocated — porting it would make this port's HARD refusal reject jobs that fit. Derived `vvvv`: `gth-szv` 2×2×2 **2.0 MiB**, 4×4×4 **1.0 GiB**; `gth-dzvp` 2×2×2 **1.79 GiB**, 3×3×3 **68.7 GiB**; ×3 for KUCCSD's three `Wvvvv`-class tensors, **×16 for KGCCSD**. Every `§9.2` fixture is `gth-szv`, where `vvvv` at 2×2×2 is 2 MiB — so a gate on those alone ships the HDF5 spill path never once executed, which is 17-12's exit-137 shape. Gated: `crates/pyscf-runtime/tests/zworkspace_pool.rs`, `crates/pyscf-pbc-lib/tests/symm_map.rs`, `crates/pyscf-pbc-cc/tests/{ktensor,keris_tiers,kccsd_rhf,kccsd_t}.rs`. |
| **D-PBC-35** | **Unported `pyscf.pbc` families: ANNOUNCED passthrough.** `python/pyscf/pbc/__init__.py` keeps `pkgutil.extend_path`, so any `pyscf.pbc.*` module the overlay does not shadow still resolves to the next `pyscf` on `sys.path` (upstream Python). A `sys.meta_path` finder (`python/pyscf/pbc/_unported.py`) that only announces — `find_spec` always returns `None` — emits ONE `PbcUpstreamFallthroughWarning` (a `UserWarning`) per family per process the first time an import lands outside the overlay, naming the family, its status and the resolved file; overlay shims that fall through for unported names (`lib.kpts_helper`, `lib.kpts`, `tools`, `tools.pbc`, `dft.gen_grid`) announce through the same one-per-family set. `pyscf.pbc.which_impl(name)` returns `"native"` / `"partial"` / `"upstream"` for a family or dotted name (`"refused"` for dotted names bound as raising stubs), from the registry `FAMILIES`: **10 upstream** (`grad`, `geomopt`, `tdscf`, `tddft`, `gw`, `adc`, `x2c`, `eph`, `mpicc`, `mpitools`), **6 partial** (`scf`, `tools`, `gto`, `symm`, `ci`, `lib`), **5 native** (`df`, `dft`, `mp`, `cc`, `ao2mo`). Overlay names are always the SAME `_native` objects, never wrappers; upstream's `isinstance(kpts, KPoints)` dispatch lives in the native constructors (20-12 D1) and the function-shaped `RHF`/`UKS`/`KS`/`KKS` dispatch in the overlay packages. Status from the Python user's point of view: Phase-19 crates (`tdscf`, `gw`, `adc`, `x2c`, `eph`) and Phase-18 `grad` have Rust code but are unbound, so they are `"upstream"`. User-facing table: `docs/pbc-status.md`. Recorded 2026-09-14, plan 20-17. | The plan's default, and the escalation trigger (a warning breaking a test that asserts clean stderr) did not fire: no test in `python/pyscf/tests` asserts on warnings or stderr, and the full suite shows no new failure. **Hard refusal** was rejected because it breaks every upstream script that touches an unported family; **silent passthrough** was the status quo (measured 2026-09-12: `pyscf.pbc.scf` resolved to the vendored tree) and is what let a vacuous Phase-20 gate stand. **Measured caveat, recorded so nobody mistakes "passthrough" for "works"** (`20-pbc-python-bindings/measurements/overlay-passthrough.md`): over the 195 modules of the vendored `pyscf/pbc` tree, **154 of the 177 the overlay does not shadow fail to import** under the overlay (41/195 import after 20-17, 31/195 before, no regression), identically whether the fallthrough target is site-packages 2.14.0 or vendored 2.12.1 — the MOLECULAR overlay packages shadow upstream's `pyscf.gto`/`scf`/`dft`/`cc`/`mp`/`grad` without the names upstream imports. Of the ten upstream families only `x2c`, `eph`, `mpitools` import at the package level. And under pytest the passthrough target is `.venv` site-packages **2.14.0**, not the 2.12.1 oracle; without `PYTHONPATH=python` the venv does not load the overlay at all. |
...
| **16** | Periodic CC + CI | `KCCSD` (RHF/UHF/GHF), `kintermediates`, `KCCSD(T)`, `EOM-KCCSD` IP/EA (+EE: GHF full, RHF singlet-only, **UHF absent upstream** — see §8.8), `KCIS` | ~~`KRCCSD` `e_corr` matches upstream to 1e-8 on He 1×1×2~~ **UNMEASURED, and it contradicts `ROADMAP.md`'s 1e-14 for the same number — six orders apart, the fourth instance of this defect (cf. Phases 14, 15, 17).** Upstream's own suite asserts `KRCCSD` `e_corr` at **6 decimals** (`pbc/cc/test/test_krccsd.py:180`, `:226`, `:232`, `:338`, `:356`) and EOM roots at **3** (`:359-366`), so 1e-14 is eight orders tighter than upstream and 1e-8 is two. **Plan 16-01 measures the floor before the gate is written** (`.planning/phases/16-periodic-cc-ci/16-01-PLAN.md`) and restates it here, in `ROADMAP.md` and in `16-CONTEXT §2` together; the gate must name its DF route (`kccsd_rhf.py:37` branches on `isinstance(with_df, GDF)`, the split Phase 14 measured at 4.5e-6 Ha). **MEASURED AND RESTATED 2026-09-06** — `measurements/README.md §1`, eleven gates: `KRCCSD e_corr` **1e-7 PER DF ROUTE** (upstream's plane-wave pair self-agrees to `2.59e-7`, its Gaussian pair to `6.82e-9`, and the two pairs are `9.22e-4 Ha` apart on diamond `gth-szv` `[1,1,2]`); `KUCCSD`/`KGCCSD` vs `KRCCSD` closed-shell **1e-8** (upstream's own gap is `4.95e-9`); (T) fast-vs-slow **1e-13 relative** (upstream's own two implementations agree to `3.27e-16`); spin-orbital vs RHF (T) **1e-9** (`2.86e-10`); EOM/KCIS roots **1e-5** (upstream's own Davidson/`nroots` spread is `5.1e-7`); supercell equivalence **1e-7** (`2.97e-8`); incore-vs-spilled **bit-identical**; `symm_map`-vs-all-triples **1e-6, NOT bit-identical** (upstream's own two paths differ by `1.32e-7`); determinism at `RAYON_NUM_THREADS` 1 and 8 **bit-identical on `t1`, `t2` AND `e_corr`**. Every run at `conv_tol = 1e-9`, `conv_tol_normt = 1e-7`, `cell.mesh` pinned. **Upstream's `test_frozen_n3` anchors and every `cu_metallic` anchor are excluded**: the first FAILS on the vendored tree, the second sits in a test upstream disabled with `@unittest.skip('Results not match')` (`test_krccsd.py:403`). |
| **17** | k-point symmetry + multigrid | `pbc/symm/*`, `KPoints` IBZ machinery, all `*_ksymm` adapters, `dft/multigrid` | ~~`KRHF` with `space_group_symmetry=True` equals the no-symmetry energy to 1e-9`~~ **WRONG, replaced by five measured gates (17-01, `.planning/phases/17-ksymm-multigrid/measurements/README.md`; §8.9 below).** Gate A (IBZ integers, exact): 145/145/245/408/816/2052, reproduced bit-for-bit on `si`/`diamond`. Gate B (transforms vs one converged SCF): ≤1e-9 at default `cell.precision`, ≤1e-13 at `cell.precision=1e-13`. Gate C/D (energy, symmetry vs no-symmetry, mesh pinned): FFTDF ≤5.985e-11, GDF ≤3.433e-09, both inside upstream's own 5e-8/5e-7. Gate E (multigrid vs reference `numint`): v1 exact to 1e-12; v2 carries a mesh-independent ~2e-8…2e-7 floor and is NOT held to Gate B-D. **ACHIEVED (17-13, 2026-09-07):** A EXACT on `si` and `diamond`; B ≤1e-9 / ≤1e-13 as restated; C FFTDF `KRHF` 1.703e-11 (**20-01 note, 2026-09-13:** 1.703e-11 is NOT Gate C — `17-VERIFICATION.md:137-141` records it as the two `eig` routes, `use_ao_symmetry` on vs off, agreeing on `e_tot`; Gate C FFTDF `KRHF` is **8.793e-14**, `:123`), `KRKS` 3.109e-14, DFT+U `E_U` 6.939e-18, **GDF `KRKS` 1.432e-06 NOT MET and recorded, not absorbed**; **D NOT RUN as a Rust gate** — 17-01's upstream-vs-upstream numbers stand, this phase's own two-route comparisons are tighter than any port-vs-upstream one could be given the 1.282e-06 mean-field residual measured on the He CC fixture, and it is a carry-over rather than a pass; E v2 `get_j` 1.24e-8 (diamond) / 6.80e-8 (si), at 17-01's upstream floors. Post-SCF: `KMP2` ksymm-vs-full-BZ **1.138e-10** (FFTDF) / **5.632e-11** (GDF); `KRCCSD` ksymm vs this port's own full BZ from ONE mean field **4.838e-12**. |
| **18** | Periodic gradients + stress + geomopt | `grad/krhf`, `kuhf`, `krks`, `kuks`, `krkspu`, `kukspu`, gamma variants, `*_stress`, `geomopt` | **RESTATED 2026-09-20 by 18-21 (`measurements/README.md`):** **A** strain components vs own FD at **A1 1e-9 / A2 2e-9 / A3 1e-8** (floors 9.91e-10 / 5.01e-10 / 3.42e-9); **B** analytic gradient vs `verify_fd` at **1e-6 Ha/Bohr** (**5e-6** DFT+U; floors ~4–5e-10); **C** vs upstream `lib.fp(g)` at upstream's decimals; **D** stress vs FD of `E(ε)` at **1e-6 Ha/Bohr³** (`h = 1e-3`, `/vol` in the assertion); **E** gamma multigrid-v2 bodies at **1e-8** (floors 9.1e-11 … 6.24e-9). The five `_ip1_`/`_ip2` PP-gradient symbols ship under the existing `gth-pp` feature (18-CONTEXT §1.3 retires the Wave-0.5 framing and §2.4's BLOCKS rows) |
| **19** | Periodic response + relativistic | `tdscf` (k + gamma), `gw` (AC + CD), `adc` (IP/EA), `x2c`, `newton_ah`, `stability`, `cphf`, `eph` | ~~KRHF-TDA lowest excitation matches upstream to 1e-6 eV~~ **MEASURED AND RESTATED 2026-09-12** (19-01, nineteen plans — §8.11's eight was an undercount): **A1** HF-family roots 1e-5; **A2** KS-family roots 1e-8 Ha; **B** single-CPHF seam (structural); **C** G0W0 1e-4 per route (AC ≠ CD); **D** ADC IP/EA 1e-4; **E** X2C 1e-8 Ha |
| **20** | MPI + tools + bindings + ship | `mpicc`/`mpitools` (feature-gated), `k2gamma`, `lattice`, `pyscf_ase`, `pywannier90` shim, full `pyscf.pbc.*` PyO3 surface, oracle CI, benchmarks (**scope restated by `20-CONTEXT.md §5`: eighteen plans replace §8.12's eight; `mpicc` dropped — it binds the stub `pyscf-pbc-mpi`**) | ~~`from pyscf.pbc import gto, scf, dft` runs an unmodified upstream script~~ **VACUOUS — true today with zero work, because `python/pyscf/pbc/__init__.py` is a `pkgutil.extend_path` passthrough to the vendored upstream tree (`20-CONTEXT.md §1.1`). RESTATED 2026-09-13 (20-01), enforced by 20-18:** the identity contract `assert pyscf.pbc.scf.KRHF is pyscf._native.pbc.scf.KRHF` holds for all 18 names in `python/pyscf/tests/test_pbc_identity_gate.py`, AND the unmodified upstream scripts `examples/pbc/20-k_points_scf.py` and `examples/pbc/22-k_points_mp2.py` run with that identity holding, on the floors of `.planning/phases/20-pbc-python-bindings/measurements/README.md`. Discriminates: 18/18 fail on the pre-binding tree (`measurements/identity-gate-pre-20-09.out`). |
...
(`src/padding.rs:35/:66/:96`, `src/frozen_k.rs:86/:140/:192`), so 16-04's
Task 1 stop-condition is satisfied and no second padding implementation is
written. `KptsHelper::build_symm_map` / `transform_symm` also shipped with
Phase 15 (`crates/pyscf-pbc-lib/src/khelper.rs`), so 16-02 Task 2 is a test
task, not a build task.

**Phase 16 was HARD-BLOCKED on Phase 15** (`16-CONTEXT §1.1`): all nine k-point
...
`crates/pyscf-pbc-mp` was a 13-line stub. Waves 0 (16-01/02/03) have no such
...
**PHASE CLOSED 2026-09-07** — `.planning/phases/17-ksymm-multigrid/17-VERIFICATION.md`
is the authority for every number below.

| Plan | Content | Port from | Status (2026-09-07) |
|---|---|---|---|
| **17-01** | Measure the floor; fixtures; restate every gate (Gates A-E) in `ROADMAP.md`, this file and `17-CONTEXT.md` together | — (measurement only, no Rust) | **MEASURED** |
| **17-02** | `symm/geom.py` + `tables.py` + `group.py` (`PGElement`, `PointGroup`, `Representation`); records D-PBC-25 (crate-layering ruling) | `symm/geom.py`, `tables.py`, `group.py` (821 l) | **SHIPPED** |
| **17-03** | `symm/space_group.py` + `symmetry.py` (`Symmetry` base); `Cell` symmetry fields, `symmetrize_mesh`, `build_lattice_symmetry` | `space_group.py` + `symmetry.py` (717 l) | **SHIPPED** |
| **17-04** | `symm/basis.py` — `symm_adapted_basis`, `Cell::_build_symmetry` (**omitted from the original 8-plan table**; the default `use_ao_symmetry=True` SCF path needs it) | (161 l) | **SHIPPED** |
| **17-05** | `lib/kpts.py` `KPoints`: `make_kpts_ibz`, `stars`, `bz2ibz`, `addition_table`, `inverse_table`, `get_kconserv`, `transform_*` (dm/fock/mo_coeff/mo_energy); closes `make_kpts_with_symmetry`; `is_trim` | `lib/kpts.py` (1223 l) | **SHIPPED** |
| **17-06** | `lib/ktensor.py` `KsymmArray` (**moved here by `15-CONTEXT §1.1`**; the original table never recorded the move) | (386 l) | **SHIPPED**; acceptance handoff CLOSED 3.3e-12 |
| **17-07** | `khf_ksymm`/`kuhf_ksymm`/`kghf_ksymm` as `KOverrideHooks` impls; records D-PBC-26 (the IBZ-restricted `get_jk` fast path — see below) | (840 l) | **SHIPPED**; `eig_trs`/KUHF/KGHF and Gate D carried over; Gate C's `KRHF` half closed by 17-09's fixture |
| **17-08** | `krks_ksymm`/`kuks_ksymm`/`krkspu_ksymm`/`kukspu_ksymm` | (422 l) | **SHIPPED** Tasks 1-4 + `kukspu_ksymm`; GDF Gate C **FAILS 1.432e-06**, recorded |
| **17-09** | `kmp2_ksymm` (+ `kccsd_rhf_ksymm` if Phase 16 has shipped `KRCCSD` by then — otherwise defer the CC half explicitly, do not guess) | 285 l (+1071 l if Phase 16 has landed) | **SHIPPED IN FULL** — both halves; `sym="s2"` landed with it |
| **17-10** | `ft_ao._RangeSeparatedCell` + `ExtendedMole` (**omitted from the original 8-plan table entirely**; closes `exclude_dd_block`/`strip_basis`/the Phase-13 `ft_aopair` residual for Phase 14; independent track, no shared code/fixture/gate with the rest of the phase, can run in parallel from wave 1) | ~600 l of `ft_ao.py` | **SHIPPED** incl. Task 4 (band route bit-identical) |
| **17-11** | multigrid **v1** — `multigrid.py`, 2 cubecl kernels (`NUMINT_fill`, `NUMINT_rho_drv`) | 1962 l | **SHIPPED**, v1 Gate E at machine precision |
| **17-12** | multigrid **v2** — `multigrid_pair.py` + `pp.py` + `utils.py`, 12 cubecl kernels via `_backend_c.py`; this is the half Phase 18's `grad/rhf.py`/`grad/uhf.py` assert `isinstance(ni, MultiGridNumInt2)` on | 1257+256+70 l | **COMPLETE** — kernel 8/8, host Gate E 10/10 |
| **17-13** | Verification against the restated gates (Gates A-E, D-PBC-26's fast-path equivalence, the multigrid speed corollary) | — | **WRITTEN** — `17-VERIFICATION.md` |
...
| Plan | Content | Port from |
|---|---|---|
| **18-01** | Gradient integral infrastructure: `pbc_intor` derivative families (`int1e_ipovlp`, `int1e_ipkin` — both cintx-ready) over lattice images; `vpploc_part2_nuc_grad`, `vppnl_nuc_grad` (the `_ip1_`/`_ip2` PP-gradient symbols ship under the existing `gth-pp` feature — the `needs cintx` framing retired 2026-09-13 by 18-03 per 18-CONTEXT §1.3), `ewald_nuc_grad` | `pp_int.py:171-210, 300-407, 443-510`, `ewald_methods.py:101-122, 256-292` |
...
| **18-07** | Verification (RESTATED 2026-09-20 by 18-21 — the 1e-5 Ha stress number was the wrong unit, upstream's stress is a pressure): five gates — **A** strain components vs own FD **A1 1e-9 / A2 2e-9 / A3 1e-8**; **B** every gradient body vs `verify_fd` at **1e-6 Ha/Bohr** (**5e-6** DFT+U); **C** vs upstream `lib.fp(g)` at upstream's decimals; **D** stress vs FD of `E(ε)` at **1e-6 Ha/Bohr³** with `/vol` in the assertion; **E** gamma multigrid-v2 bodies at **1e-8**, never Gate B's number. Floors in `measurements/README.md` | |
...
**Plans:** 19 (`19-CONTEXT.md §3`; §8.11's original eight was an undercount —
its 19-07 row packed 3,437 lines of ADC into one plan — corrected 2026-09-12).
