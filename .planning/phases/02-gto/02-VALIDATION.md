...
| GTO-05 | ECP loading (parser ships); `int1e_ecp` evaluation closes via gap-closure (D-06) | unit (loading) + ignored (evaluation) | `cargo test -p pyscf-gto --test ecp_load`; `int1e_ecp` test marked `#[ignore = "Pending cintx ECP merge"]` | ⚠️ partial — loading ✅ (plan 02-07: format_ecp + make_ecp_env shipped + `EcpEngineNotAvailable` stub); eval ⬜ pending — see plan 02-10 (gap-closure when cintx ECP merges) |
...
|----------|-------------|------------|-------------------|
| `int1e_ecp` numerical accuracy vs upstream | GTO-05 (evaluation half) | Blocked on cintx ECP merge (D-06 parallel sequencing). Plan 02-10 ships the gap-closure that flips the `EcpEngine` impl from `EcpEngineNotAvailable` to a cintx-backed engine when cintx ships int1e_ecp Type-1 + Type-2. | After cintx ECP lands, plan 02-10 removes the `#[ignore]` on the ECP eval test and runs `cargo test -p pyscf-gto int1e_ecp_oracle --features release-oracle`. Result must match upstream `mol.intor('int1e_ecp')` to oracle tolerance. |
...
**Approval:** approved 2026-05-10 — plan 02-09 oracle harness shipped + all in-scope REQ-IDs flipped to ✅ or ⚠️ partial-with-explicit-deferral. Remaining ⬜ pending (deferred) entries are GTO-05 evaluation (plan 02-10) and GTO-07 l ≥ 1 (Phase 4 DFT) per ROADMAP scope.
