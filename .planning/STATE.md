...
16-04, 16-05, 16-06, 16-07, 16-08, 16-09, 16-10, 16-11, 16-12's
`kuccsd_rdm`, 16-13 and this verification). Every plan is now started; what
remains is recorded per plan with its reason and its unblocking
...
Previous session: 2026-09-20
Stopped at: **Phase 18 IMPLEMENTED, NOT CLOSED** — `.planning/phases/18-periodic-gradients-stress-geomopt/18-VERIFICATION.md` is the authority (18-15). All 21 plans shipped: 18-08 (`pyscf-pbc-grad/src/krkspu.rs`/`kukspu.rs` — raw-projection Löwdin-derivative C1 vs FD 3.267e-7 reproducing upstream's own 3.266e-7, U-vs-E_U-FD 2.4e-10, closed-shell limit exact, wiring exact) and 18-14 (`pyscf-pbc-geomopt/src/geometric_solver.rs` — native BFGS+RFO reuse, 4/4 machinery green, atoms-only, lattice half of ROADMAP:464 restated) landed this session; 18-21 restated gates A–E with measured floors into `measurements/README.md`, `18-CONTEXT §2.3`, `ROADMAP:464`, `PBC-MASTER-PLAN §7/§2.4/§8.10`. RUN/FAIL with numbers: gamma Gate E (F-18-10-01, O(0.1–0.5)), KRHF 1×1×2 Gate B 3.323e-6 (F-18-19-01), KUHF open-shell Gate B 1.282e-4 bit-identical debug/release (F-18-06-01), lif closed-form A1 3.74e-9/1.08e-9 (F-18-12-01), 2-atom geomopt stall at 4.6e-2 with stretch converged (F-18-14-01, transverse outside the molecular internal span — faithful, upstream agrees to 7e-9). NOT RUN: full-SCF+U B/C (no driver, C-18-08-01). Incidental: 1-line `svml_pow8` debug_assert fix (bit-pattern `as f64` → `f64::from_bits` — un-broke every debug test); a repo-wide rustfmt churn found mid-session was fully reverted. External blockers: lapack_rs `zheevd` DC panic for nao ≥ 16 (no SCF gates on bigger cells); `geometric` absent from `.venv`. ROADMAP Phase-18 box stays `[ ]`.
