...
- [x] **GEOMOPT-01**: `pyscf.geomopt.optimize(mf)` runs a native Rust BFGS+RFO optimizer in redundant internals; no Python `geomeTRIC` or `pyberny` runtime dependency
- [x] **GEOMOPT-02**: `pyscf.geomopt.geometric_solver.optimize(mf)` is a drop-in shim that delegates to the native optimizer (preserves the canonical PySCF import path) — _Rust-side shim surface in 07-06 (`geometric_solver::{kernel,optimize}` over the ONE native engine, D-06); the Python entry point + the canonical import path COMPLETE in 07-09 (the _native.geomopt PyO3 bridge)_
- [x] **GEOMOPT-03**: `pyscf.geomopt.berny_solver.optimize(mf)` is also a drop-in shim — _Rust-side shim surface in 07-06 (`berny_solver::{kernel,optimize}` — a thin alias over the SAME native engine, T-07-20); the Python entry point COMPLETE in 07-09 (the _native.geomopt PyO3 bridge)_
...
| GEOMOPT-01 | Phase 7 | Complete |
| GEOMOPT-02 | Phase 7 | Complete (07-06: Rust shim surface — geometric_solver::{kernel,optimize} over the ONE native engine, D-06; 07-09: the Python pyscf.geomopt.geometric_solver.optimize(mf) entry point + the canonical import path via the _native.geomopt bridge) |
| GEOMOPT-03 | Phase 7 | Complete (07-06: Rust shim surface — berny_solver::{kernel,optimize}, a thin alias over the SAME engine, T-07-20; 07-09: the Python pyscf.geomopt.berny_solver.optimize(mf) entry point via the _native.geomopt bridge) |
