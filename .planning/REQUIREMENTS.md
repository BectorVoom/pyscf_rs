...
- [~] **SCF-05**: `mf.init_guess` accepts `'minao'`, `'atom'`, `'1e'`, `'huckel'`, `'chkfile'`; user-supplied `dm0` is respected (plans 03-11/03-06/03-13 — `'1e'`, `'chkfile'`, user-`dm0`, AND the DEFAULT `'minao'` are implemented: `init_guess_by_minao` byte-matches the upstream H2 docstring dm `[[0.94758917,0.09227308],…]` and converges RHF/DF-HF out-of-the-box, via the new `pyscf_gto::intor_cross` + the ported `NRSRHF_CONFIGURATION`/`frac_occ`; `crates/pyscf-scf/tests/init_guess_minao.rs`. `'atom'`/`'huckel'` remain NotYetImplemented. HEAVY-ATOM CAVEAT RESOLVED via 02-11: the `Tr(dm·S)≈7.9` deficit was the general-contraction PARSER TRUNCATION (ANO O S-block loaded as 1 contraction instead of 8). After 02-11's parser + cintx-coefficient-layout fix, H2O minao `Tr(dm·S)≈9.86` — the correct heavy-atom projection. The residual sub-nelec value is the INHERENT minao non-normalization (upstream keeps `dm *= nelec/(dm·s).sum()` commented out; the byte-matched H2 docstring dm itself traces to 1.976, not 2.0), NOT a data gap)
