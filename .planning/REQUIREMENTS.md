...
- [ ] **GTO-01**: `pyscf.M(...)` factory and `gto.Mole` class accept all 5 atom-input forms (string, list-of-tuples, list-of-lists, file path, geom callable)
...
- [ ] **GTO-08**: `Mole` exposes the ≥30 attribute floor (`atom`, `basis`, `charge`, `spin`, `nelectron`, `natm`, `nbas`, `nao_nr`, `nao_2c`, `ao_loc_nr`, `ao_labels`, `cart`, `verbose`, `max_memory`, `unit`, `output`, `_atm`, `_bas`, `_env`, …) with semantics matching upstream
...
- [ ] **SCF-05**: `mf.init_guess` accepts `'minao'`, `'atom'`, `'1e'`, `'huckel'`, `'chkfile'`; user-supplied `dm0` is respected
- [ ] **SCF-06**: `mf.level_shift`, `mf.damp`, `mf.max_cycle`, `mf.conv_tol`, `mf.conv_tol_grad` controls match upstream semantics
- [ ] **SCF-07**: `mf.density_fit(auxbasis=...)` returns an SCF object that solves DF-HF; auxbasis defaults match upstream (`weigend`, `cc-pvdz-jkfit`)
